...
import DateRangeFilter from '@/components/features/links/DateRangeFilter';
...
export default function Analytics() {
  ...
  const [selectedLink, setSelectedLink] = useState<LinkPerformance | null>(null);
  ...
  const [trafficData, setTrafficData] = useState<{ name: string; clicks: number }[]>([]);
  const [geoData, setGeoData] = useState<GeoMetric[]>([]);
  const [deviceData, setDeviceData] = useState<DeviceMetric[]>([]);
  const [browserData, setBrowserData] = useState<BrowserMetric[]>([]);
  const [osData, setOSData] = useState<OSMetric[]>([]);
  const [referrersData, setReferrersData] = useState<ReferrerMetric[]>([]);
  const [peakHoursData, setPeakHoursData] = useState<PeakHourMetric[]>([]);
  const [linkPerformanceData, setLinkPerformanceData] = useState<LinkPerformance[]>([]);
  ...
  const processData = (clicks: (Click & { links: Link })[], links: Link[]) => {
    // --- Global Stats ---
    ...
    const avgSession = 'N/A'; // Placeholder
    ...
    const performanceData = Array.from(linkStats.values()).map(link => ({
        ...
        status: (link.is_active ? 'Active' : 'Paused') as 'Active' | 'Paused',
        trend: 0, // Mock
        trendUp: true // Mock
